...
export function createServer(client = new SiyuanClient()) {
    const server = new Server({ name: "siyuan-mcp", version: SERVER_VERSION }, { capabilities: { tools: {}, resources: {} } });
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: toolDefinitions,
    }));
    server.setRequestHandler(CallToolRequestSchema, async (request) => invokeTool(client, request.params.name, request.params.arguments));
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async ({ params: { uri } }) => {
        try {
            if (uri === "siyuan://recent") {
                const data = await client.json("/api/query/sql", {
