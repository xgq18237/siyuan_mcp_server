...
const string = (description: string, extra: JsonSchema = {}): JsonSchema => ({
  type: "string",
  description,
  ...extra,
});
const boolean = (description: string): JsonSchema => ({ type: "boolean", description });
const number = (description: string, extra: JsonSchema = {}): JsonSchema => ({
  type: "number",
  description,
  ...extra,
});
const array = (description: string, items: JsonSchema): JsonSchema => ({
  type: "array",
  description,
  items,
});
const object = (description: string, additionalProperties: boolean | JsonSchema = true): JsonSchema => ({
  type: "object",
  description,
...
function asArguments(value: unknown): Arguments {
...
function requiredString(args: Arguments, key: string): string {
  ...
  if (typeof value !== "string" || value.length === 0) throw new Error(`缺少有效参数：${key}`);
  return value;
}
...
function requiredArray(args: Arguments, key: string): unknown[] {
  const value = args[key];
  if (!Array.isArray(value)) throw new Error(`缺少数组参数：${key}`);
  return value;
}

function optionalNumber(args: Arguments, key: string, fallback: number): number {
  const value = args[key];
...
function allowedWritePrefixes(): string[] {
...
function assertWritableWorkspacePath(value: string): void {
  const normalized = value.replace(/\\/g, "/");
  const allowed = allowedWritePrefixes().some((prefix) =>
    normalized === prefix || normalized.startsWith(`${prefix}/`));
  if (!allowed) {
    throw new Error(
      `工作空间写入路径不在允许范围：${value}。可通过 SIYUAN_MCP_WRITE_PATH_PREFIXES 配置。`,
    );
  }
}
...
function findInsertedBlockId(value: unknown): string | undefined {
  ...
  if (record.action === "insert" && typeof record.id === "string") return record.id;
...
function stringArray(value: unknown[], key: string): string[] {
  ...
  if (strings.length !== value.length) throw new Error(`${key} 必须全部是非空字符串`);
  return strings;
}

async function renameDatabase(client: SiyuanClient, avID: string, name: string): Promise<unknown> {
...
async function renderDatabase(client: SiyuanClient, args: Arguments): Promise<unknown> {
...
export async function invokeTool(
  ...
  try {
    switch (name) {
      case "list_notebooks": {
        const data = await client.json<{ notebooks: unknown[] }>("/api/notebook/lsNotebooks");
      ...
      case "insert_block": {
        ...
        if (!nextID && !previousID && !parentID) throw new Error("nextID、previousID、parentID 至少提供一个");
      ...
      case "move_block": {
        const previousID = optionalString(args, "previousID");
        const parentID = optionalString(args, "parentID");
        if (!previousID && !parentID) throw new Error("previousID 或 parentID 至少提供一个");
      ...
      case "create_database": {
        ...
        try {
          ...
          if (!blockID) throw new Error("无法从插入事务中取得数据库块 ID");
          ...
          if (nameArg) await renameDatabase(client, avID, nameArg);
          ...
          throw new Error(
            `${error instanceof Error ? error.message : String(error)}${blockID ? `（数据库块 ${blockID} 已尝试回滚）` : ""}`,
          );
        }
      }
      case "get_database":
      ...
      case "rename_database":
        return success(await renameDatabase(
          client,
          requiredString(args, "avID"),
      ...
      case "append_database_rows": {
        ...
        const rawAv = await client.json<{ av?: { keyValues?: Array<{ key?: { id?: string; type?: string } }> } }>(
          "/api/av/getAttributeView",
          { id: avID },
        );
        const primary = rawAv.av?.keyValues?.find((item) => item.key?.type === "block")?.key;
        if (!primary?.id) throw new Error("数据库主键字段不存在");
      ...
      case "put_file": {
        ...
        const fields: Record<string, string> = { path: targetPath };
        if (typeof args.isDir === "boolean") fields.isDir = String(args.isDir);
        if (typeof args.modTime === "number") fields.modTime = String(args.modTime);
        const files: Array<{
          field: string;
          filePath?: string;
          data?: Uint8Array;
      ...
      case "upload_asset": {
        ...
        return success(await client.multipart(
          "/api/asset/upload",
          { assetsDirPath: requiredString(args, "assetsDirPath") },
          filePaths.map((filePath) => ({ field: "file[]", filePath })),
        ));
      }
      ...
      case "check_siyuan_status": {
        const [version, notebooks, sql] = await Promise.all([
          ...
          client.json<{ notebooks: unknown[] }>("/api/notebook/lsNotebooks"),
